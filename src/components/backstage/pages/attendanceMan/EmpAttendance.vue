<template>
  <div class="allatt">
    ...
    <div class="table">
      <el-table
        :data="pagingStaffInfo" border size="mini" :row-style="rowStyle" :cell-style="cellStyle" :header-row-style="headerRowStyle" class="stable"
  ...
  export default {
    ...
    methods:{
      ...
      getEmp:function(){
        ...
        this.departmentInfo=[];
        this.postInfo=[];
        axios.get('/api/getEmpAttend').then(response=>{
          ...
          str=data[i].a_time.substring(0,10);
            this.allStaffInfo.push({a_time:str,emp_id:data[i].emp_id,emp_name:data[i].emp_name,dep_name:data[i].dep_name,job_name:data[i].job_name,start_time:data[i].start_time,end_time:data[i].end_time});
          }
          }
        });
